...
.user_table th:nth-child(1), .user_table td:nth-child(1) {
	width: 10%;
}

.user_table th:nth-child(2), .user_table td:nth-child(2) {
	width: 70%;
}
		...
		$('#searchForm')
				.on(
						...
						function(e) {
							...
							const url = '../common/component/mypage/myQuestion.jsp?type='
		...
		$('tbody')
				.on(
						...
						function(e) {
							...
							if (inqNum) {
								$contentDiv
										.load('../common/component/mypage/myQuestionDetail.jsp?inq_num='
												+ inqNum);
							}
						});
		...
		<div class="search-container">
			<div class="search-box">
				<form id="searchForm">
					...
					</select> <input type="text" name="keyword" id="searchInput"
						class="searchText" placeholder="검색어를 입력해주세요" value="${keyword}" />
		...
		<div class="pagination">
			...
			<c:choose>
				<c:when test="${paging.page > 1}">
					<a
						href="../common/component/mypage/myQuestion.jsp?page=${paging.page - 1}&type=${type}&keyword=${keyword}">◀</a>
				</c:when>
				<c:otherwise>
					<a class="disabled">◀</a>
				</c:otherwise>
			...
			<c:forEach var="i" begin="${paging.startPage}"
				...
				<c:choose>
					...
					<c:otherwise>
						<a
							href="../common/component/mypage/myQuestion.jsp?page=${i}&type=${type}&keyword=${keyword}">${i}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
