...
<div class="headerTop">
	<div class="header_content main-wrapper">
		<h5 class="logo" title="모두쉼로고">
			<a href="http://localhost/sistSndPjr/user/main/user_main_page.jsp"><img
				src="http://localhost/sistSndPjr/user/common/images/logo251.png" alt=""
				class="logo-img"></a>
		</h5>
		<div class="member">
			<ul class="login_menu center">
				<li class="<c:if test='${menu eq "facility"}'>active</c:if>"><a
					href="http://localhost/sistSndPjr/user/main/rest_area_facility.jsp">휴게소
						편의시설 정보</a></li>
				<li class="<c:if test='${menu eq "gas"}'>active</c:if>"><a
					href="http://localhost/sistSndPjr/user/main/gas_station.jsp">노선별주유소</a>
				</li>
				<li class="<c:if test='${menu eq "board"}'>active</c:if>"><a
					href="http://localhost/sistSndPjr/user/main/user_board.jsp">게시판</a></li>
				<li class="<c:if test='${menu eq "mypage"}'>active</c:if>"><a
					href="http://localhost/sistSndPjr/user/main/user_mypage.jsp" id="mypage">마이페이지</a></li>
			</ul>
			<ul class="login_menu right">
				<c:choose>
					<c:when test="${loginCheck }">
						<li><a href="http://localhost/sistSndPjr/user/main/user_mypage.jsp" class="no-hover"><span style="color: #0000FF;">${name }</span>님</a></li>
