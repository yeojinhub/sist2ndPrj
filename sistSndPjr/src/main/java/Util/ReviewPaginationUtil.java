...
public class ReviewPaginationUtil {
	...
	public static String pagination(PaginationDTO pDTO) {
		...
		if (currentPage > pageNumber) { // 시작페이지보다 1 적은 페이지로 이동
			...
			prevMark.append("<a href='").append(url).append("?currentPage=").append(movePage).append("' class='prev'>◀</a>");
		} // end if
		...
		while (movePage <= endPage) {
			if (movePage == currentPage) { // 현재 페이지는 링크를 설정하지 않고 span태그로 색상을 조정한다.
				...
				pageLink.append("<a href='?currentPage=").append(movePage).append("' style='text-decoration: none; color: #333;' class='number'>").append(movePage).append("</a>");
			} // end if-else
			movePage++;
		} // end while
		...
		if (totalPage > endPage) {
			...
			nextMark.append("<a href='").append(url).append("?currentPage=").append(movePage).append("' class='next' style='margin-left: -10px;'>▶</a>");
